...
<div class="w-full max-w-[700px] mx-auto flex flex-col" style="height: calc(100vh - 80px);">
    ...
    <div class="flex items-center gap-3 px-4 py-3 border-b border-[#e2e8f0] bg-white rounded-t-2xl">
        <a href="{{ route('patient.dashboard') }}" class="text-[#64748b] hover:text-[#0D9488] transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <div class="w-9 h-9 rounded-full bg-[#0D9488]/10 flex items-center justify-center flex-shrink-0">
...
<script>
    ...
    const chatUrl = '{{ route("patient.chat-messages", $doctor) }}';
    const sendUrl = '{{ route("patient.chat-send", $doctor) }}';
