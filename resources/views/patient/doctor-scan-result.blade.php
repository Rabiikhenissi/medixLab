<x-layouts.patient>
    <x-slot:title>{{ __('patient.scan_result.title') }} — Medix eSanté</x-slot:title>
    ...
    <div class="w-full max-w-[700px] mx-auto">
        <div class="glass-card rounded-[20px] p-8 md:p-10 relative overflow-hidden">
            ...
            <div class="flex items-center space-x-4 border-b border-[#e2e8f0]/80 pb-6 mb-8 select-none">
                ...
                <div>
                    <h1 class="text-xl font-bold text-[#1e293b]">{{ __('patient.scan_result.title') }}</h1>
                    <p class="text-sm text-[#64748b] mt-0.5">
                        {{ __('patient.scan_result.subtitle') }}
                    </p>
                </div>
            </div>
            ...
            @if ($existing && $existing->access_status === 'blocked')
                <div class="mb-6 flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-xl">
                    ...
                    <div>
                        <p class="text-sm font-bold text-red-700">{{ __('patient.scan_result.doctor_blocked') }}</p>
                        <p class="text-xs text-red-600 mt-1">{{ __('patient.scan_result.doctor_blocked_hint') }}</p>
                    </div>
                </div>
            @elseif ($existing && $existing->access_status === 'granted')
                <div class="mb-6 flex items-start gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
                    ...
                    <div>
                        <p class="text-sm font-bold text-green-700">{{ __('patient.scan_result.already_linked') }}</p>
                        <p class="text-xs text-green-600 mt-1">{{ __('patient.scan_result.already_linked_hint') }}</p>
                    </div>
                </div>
            @endif
            ...
            <div class="bg-white border border-[#e2e8f0] rounded-2xl p-6 shadow-xs">
                <div class="flex items-center gap-4">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-[#64748b] mt-2 font-mono font-semibold">{{ __('patient.scan_result.code') }} {{ $doctor->doctor_code }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6">
                    ...
                    <div class="flex items-center gap-2 text-xs text-[#64748b]">
                        ...
                        <span>{{ $doctor->user->phone ?? __('patient.scan_result.phone_not_provided') }}</span>
                    </div>
                </div>
            </div>
                ...
                @else
                    <div class="mb-4 flex items-start gap-3 p-4 bg-[#0D9488]/5 border border-[#0D9488]/20 rounded-xl">
                        ...
                        <p class="text-xs text-[#475569] leading-relaxed">
                            {{ __('patient.scan_result.confirm_info') }}
                            <span class="font-bold text-[#0D9488]">{{ __('patient.scan_result.immediate_link') }}</span>
                        </p>
                    </div>
                    <form action="{{ route('patient.scan-doctor-link', $doctor->doctor_code) }}" method="POST">
                        ...
                        <button type="submit"
                            ...
                            {{ __('patient.scan_result.become_patient') }}
                        </button>
                    </form>
                    <a href="{{ route('patient.dashboard') }}"
