<x-layouts.patient>
    ...
    <div class="max-w-5xl mx-auto space-y-6">
        <div>
            <h1 class="text-2xl font-bold text-[#1e293b]">{{ __('patient.invoices.title') }}</h1>
            <p class="text-sm text-[#64748b] mt-1">{{ __('patient.invoices.subtitle') }}</p>
        </div>
                ...
                <div class="flex items-start justify-between">
                    <div class="space-y-1">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase
                                ...
                                {{ $invoice->status === 'paid' ? __('patient.invoices.status_paid') : ($invoice->status === 'partially_paid' ? __('patient.invoices.status_partial') : ($invoice->status === 'cancelled' ? __('patient.invoices.status_cancelled') : __('patient.invoices.status_pending'))) }}
                            </span>
                        </div>
                        <p class="text-xs text-[#64748b]">
                            <span class="font-medium">{{ $invoice->labo->name ?? __('patient.invoices.laboratory') }}</span>
                    ...
                    <div class="text-right">
                        ...
                        @if($invoice->paid_amount > 0)
                            <p class="text-[11px] text-emerald-600 font-semibold">{{ __('patient.invoices.paid') }}: {{ number_format($invoice->paid_amount, 3) }} TND</p>
                        @endif
                    </div>
                </div>
        ...
        @empty
            <div class="bg-white border border-[#e2e8f0] rounded-2xl p-12 text-center">
                ...
                <p class="text-sm text-[#94a3b8] font-medium">{{ __('patient.invoices.empty') }}</p>
            </div>
        @endforelse
