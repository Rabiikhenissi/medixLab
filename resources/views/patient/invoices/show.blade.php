    ...
    <div class="max-w-4xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-[#64748b] mt-1">{{ $invoice->labo->name ?? 'Laboratoire' }} &middot; {{ $invoice->created_at->format('d/m/Y') }}</p>
            </div>
            <div class="flex gap-2">
                ...
                <a href="{{ route('patient.invoices.index') }}" class="border border-[#e2e8f0] hover:bg-[#f8fafc] text-[#64748b] font-bold px-3 py-2 rounded-xl text-xs transition">Retour</a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden">
                    <div class="p-4 bg-[#F8FAFC] border-b border-[#e2e8f0] flex items-center justify-between">
                        <h3 class="text-xs font-bold text-[#64748b] uppercase tracking-wider">Examens</h3>
                        <span class="text-[10px] text-[#64748b]">{{ $invoice->items->count() }} article(s)</span>
                    </div>
                ...
                <div class="bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden">
                    <div class="p-4 bg-[#F8FAFC] border-b border-[#e2e8f0] flex items-center gap-2">
                        ...
                        <h3 class="text-xs font-bold text-[#7C3AED] uppercase tracking-wider">Prise en charge CNAM</h3>
                    </div>
                    <table class="w-full text-sm">
                        <thead><tr class="bg-[#F8FAFC]/50 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">
                            <th class="p-3 text-left">Examen</th>
                            <th class="p-3 text-center">Code CNAM</th>
                            <th class="p-3 text-center">Valeur B</th>
                            <th class="p-3 text-center">Taux</th>
                            <th class="p-3 text-right">Couverture</th>
                        </tr></thead>
                        <tbody class="divide-y divide-gray-100">
                            @foreach($invoice->items as $item)
                                @if($item->cnam_code)
                ...
                <div class="bg-white border border-[#e2e8f0] rounded-2xl p-4">
                    <h3 class="text-xs font-bold text-[#64748b] uppercase tracking-wider mb-2">Prescripteur</h3>
                            ...
                            <tr>
                                ...
                                <td class="p-2 text-center">
                                    @if($payment->status === 'pending')
                                        <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700">En attente</span>
                                    @else
                ...
                <a href="{{ route('patient.print-exam-request', $invoice->examRequest->id) }}" target="_blank" class="block border border-[#e2e8f0] hover:bg-[#f8fafc] rounded-2xl p-4 transition text-center">
                    ...
                    <span class="text-xs font-bold text-[#1e293b]">Voir le compte-rendu</span>
                </a>
                @endif
            </div>
            ...
            <div class="space-y-4">
                <div class="bg-white border border-[#e2e8f0] rounded-2xl p-5 space-y-3">
                    ...
                    <div class="flex justify-between text-sm"><span class="text-[#64748b]">Part CNAM</span><span class="font-bold text-[#7C3AED]">- {{ number_format(min($invoice->cnam_amount, $invoice->total_amount), 3) }} TND</span></div>
                    @endif
                    <div class="border-t border-[#e2e8f0] pt-3 flex justify-between text-sm"><span class="font-bold">Net à payer</span><span class="font-bold text-lg">{{ number_format($invoice->patient_amount, 3) }} TND</span></div>
                ...
                <div class="bg-white border border-[#e2e8f0] rounded-2xl p-5 space-y-4">
                    <h3 class="text-xs font-bold text-[#64748b] uppercase tracking-wider">Effectuer un paiement</h3>

                    <form method="POST" action="{{ route('patient.invoices.pay', $invoice->id) }}" id="payment-form">
                        @csrf

                        <div class="space-y-1.5">
                            <label class="text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Montant à payer</label>
                        ...
                        <div class="space-y-1.5 mt-4">
                            <label class="text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Méthode de paiement</label>
                            <div class="grid grid-cols-2 gap-2" id="payment-methods">
                                ...
                                <label class="payment-option cursor-pointer border-2 rounded-xl p-3 flex items-center gap-2 transition text-xs font-bold col-span-2 opacity-40 cursor-not-allowed"
                                    title="Paiement en ligne non disponible (pas de passerelle de paiement)">
                                    ...
                                    <span class="text-[#94a3b8]">Paiement en ligne</span>
