...
@section('title', __('center.billing.invoice', ['number' => $invoice->invoice_number]))

@section('content')
<div class="max-w-4xl mx-auto space-y-6 select-none">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-[#1e293b]">{{ __('center.billing.invoice', ['number' => $invoice->invoice_number]) }}</h1>
            <p class="text-sm text-[#64748b] mt-1">{{ __('center.billing.created_on', ['date' => $invoice->created_at->format(__('common.datetime_format'))]) }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden">
                <div class="p-4 bg-[#F8FAFC] border-b border-[#e2e8f0]">
                    <h3 class="text-xs font-bold text-[#64748b] uppercase tracking-wider">{{ __('center.billing.articles') }}</h3>
                </div>
                <table class="w-full text-sm">
                    <thead><tr class="bg-[#F8FAFC]/50 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">
                        <th class="p-3 text-left">{{ __('center.billing.description') }}</th>
                        <th class="p-3 text-center">{{ __('center.billing.qty') }}</th>
                        <th class="p-3 text-right">{{ __('center.billing.unit_price') }}</th>
                        <th class="p-3 text-right">{{ __('common.total') }}</th>
                        <th class="p-3 text-right">{{ __('center.billing.cnam_col') }}</th>
                    </tr></thead>
                    <tbody class="divide-y divide-gray-100">
                        @foreach($invoice->items as $item)
                        <tr>
                            <td class="p-3 font-medium">{{ $item->description }}
                                @if($item->cnam_code)<br><span class="text-[10px] text-[#7C3AED]">{{ __('center.billing.cnam_col') }}: {{ $item->cnam_code }}</span>@endif
                            </td>
            ...
            <div class="bg-white border border-[#e2e8f0] rounded-2xl p-4">
                <h3 class="text-xs font-bold text-[#64748b] uppercase tracking-wider mb-3">{{ __('center.billing.payments') }}</h3>
                @if($invoice->payments->count() > 0)
                <table class="w-full text-sm">
                    <thead><tr class="text-[10px] font-bold text-[#64748b] uppercase tracking-wider">
                        <th class="p-2 text-left">{{ __('common.date') }}</th>
                        <th class="p-2 text-left">{{ __('center.billing.method') }}</th>
                        <th class="p-2 text-right">{{ __('center.billing.amount') }}</th>
                        <th class="p-2 text-center">{{ __('common.status') }}</th>
                        <th class="p-2 text-left">{{ __('center.billing.reference') }}</th>
                    </tr></thead>
                    <tbody class="divide-y divide-gray-100">
                        @foreach($invoice->payments as $payment)
                        <tr>
                            <td class="p-2">{{ $payment->payment_date->format(__('common.datetime_format')) }}</td>
                            <td class="p-2">{{ $payment->payment_method === 'cash' ? __('center.billing.cash') : ($payment->payment_method === 'card' ? __('center.billing.card') : ($payment->payment_method === 'cheque' ? __('center.billing.cheque') : ($payment->payment_method === 'bank_transfer' ? __('center.billing.bank_transfer') : __('center.billing.online')))) }}</td>
                            ...
                            <td class="p-2 text-center">
                                ...
                                @else
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700">{{ __('center.status.confirmed') }}</span>
                                @endif
                            </td>
                ...
                <form method="POST" action="{{ route('center.payments.confirm', $pendingPayment->id) }}" class="mt-3">
                    ...
                    <button type="submit" class="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-2 rounded-xl text-xs uppercase tracking-wider transition shadow-lg shadow-amber-200 flex items-center justify-center gap-1">
                        ...
                        {{ __('center.billing.confirm_payment') }}
                    </button>
                </form>
                @endif
                @else
                    <p class="text-sm text-[#94a3b8]">{{ __('center.billing.no_payments') }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-4">
            <div class="bg-white border border-[#e2e8f0] rounded-2xl p-5 space-y-3">
                <h3 class="text-xs font-bold text-[#64748b] uppercase tracking-wider">{{ __('center.billing.summary') }}</h3>
                <div class="flex justify-between text-sm"><span class="text-[#64748b]">{{ __('common.total') }}</span><span class="font-bold">{{ number_format($invoice->total_amount, 3) }} TND</span></div>
                <div class="flex justify-between text-sm"><span class="text-[#64748b]">{{ __('center.billing.cnam_part') }}</span><span class="font-bold text-[#7C3AED]">- {{ number_format($invoice->cnam_amount, 3) }} TND</span></div>
                <div class="border-t border-[#e2e8f0] pt-3 flex justify-between text-sm"><span class="font-bold">{{ __('center.billing.patient_part') }}</span><span class="font-bold text-lg">{{ number_format($invoice->patient_amount, 3) }} TND</span></div>
                <div class="flex justify-between text-sm"><span class="text-[#64748b]">{{ __('center.billing.paid') }}</span><span class="font-bold text-emerald-600">{{ number_format($invoice->paid_amount, 3) }} TND</span></div>
                @if($invoice->balance > 0)
                <div class="flex justify-between text-sm"><span class="text-[#64748b]">{{ __('center.billing.remaining') }}</span><span class="font-bold text-amber-600">{{ number_format($invoice->balance, 3) }} TND</span></div>
                @endif
                <div class="pt-2">
                    <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase
                        ...
                        {{ $invoice->status === 'paid' ? __('center.status.paid') : ($invoice->status === 'partially_paid' ? __('center.status.payment_partial') : ($invoice->status === 'cancelled' ? __('center.status.cancelled') : __('center.status.pending'))) }}
                    </span>
                </div>
            </div>
            ...
            <div class="bg-white border border-[#e2e8f0] rounded-2xl p-5">
                <h3 class="text-xs font-bold text-[#64748b] uppercase tracking-wider mb-4">{{ __('center.billing.record_payment') }}</h3>
                <form method="POST" action="{{ route('center.billing.pay', $invoice->id) }}" class="space-y-4">
                    @csrf
                    <div>
                        <label class="text-[9px] font-bold text-[#64748b] uppercase tracking-wider mb-1.5 block">{{ __('center.billing.amount_tnd') }}</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="text-[9px] font-bold text-[#64748b] uppercase tracking-wider mb-2 block">{{ __('center.billing.payment_method') }}</label>
                        <div class="grid grid-cols-2 gap-2" id="paymentMethods">
                            <label class="payment-method flex items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition border-emerald-500 bg-emerald-50" data-value="cash">
                                ...
                                <span class="text-xs font-bold text-emerald-700">{{ __('center.billing.cash') }}</span>
                            </label>
                            <label class="payment-method flex items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition border-[#e2e8f0] bg-white hover:border-[#cbd5e1]" data-value="card">
                                ...
                                <span class="text-xs font-bold text-[#475569]">{{ __('center.billing.card') }}</span>
                            </label>
                            <label class="payment-method flex items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition border-[#e2e8f0] bg-white hover:border-[#cbd5e1]" data-value="cheque">
                                ...
                                <span class="text-xs font-bold text-[#475569]">{{ __('center.billing.cheque') }}</span>
                            </label>
                            <label class="payment-method flex items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition border-[#e2e8f0] bg-white hover:border-[#cbd5e1]" data-value="bank_transfer">
                                ...
                                <span class="text-xs font-bold text-[#475569]">{{ __('center.billing.bank_transfer') }}</span>
                            </label>
                            <label class="payment-method flex items-center gap-2 p-3 rounded-xl border-2 border-[#e2e8f0] bg-[#f8fafc] col-span-2 opacity-40 cursor-not-allowed" title="{{ __('center.billing.online_unavailable') }}">
                                ...
                                <span class="text-xs font-bold text-[#94a3b8]">{{ __('center.billing.online') }}</span>
                                <span class="text-[9px] text-[#94a3b8] font-normal ml-auto">{{ __('center.billing.coming_soon') }}</span>
                            </label>
                        </div>
                    </div>
                    <script>
                    ...
                    <div>
                        <label class="text-[9px] font-bold text-[#64748b] uppercase tracking-wider mb-1.5 block">{{ __('center.billing.transaction_ref') }} <span class="text-[#94a3b8]">({{ __('common.optional') }})</span></label>
                        <input type="text" name="transaction_id"
                            ...
                            placeholder="{{ __('center.billing.transaction_placeholder') }}">
                    </div>
            ...
            @if(in_array($invoice->status, ['pending', 'partially_paid']))
            <form method="POST" action="{{ route('center.billing.cancel', $invoice->id) }}" onsubmit="return swalConfirmSubmit(this, '{{ __('center.billing.cancel_confirm') }}')">
                @csrf
                <button type="submit" class="w-full border border-red-200 text-red-600 hover:bg-red-50 font-bold px-4 py-2 rounded-xl text-xs uppercase tracking-wider transition">{{ __('center.billing.cancel_invoice') }}</button>
            </form>
            @endif
        </div>
    </div>
