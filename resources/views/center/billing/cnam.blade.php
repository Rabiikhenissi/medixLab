...
@section('title', 'Nomenclature CNAM - Medix eSanté')

@section('content')
<div class="space-y-6 select-none">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-[#1e293b]">Nomenclature CNAM</h1>
            <p class="text-sm text-[#64748b] mt-2">Gérez les codes CNAM et les valeurs B.</p>
        </div>
        ...
        <table class="w-full text-left border-collapse min-w-[700px]">
            <thead>
                <tr class="bg-[#F8FAFC]/80 border-b border-[#e2e8f0]/80">
                    <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Code CNAM</th>
                    <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Examen</th>
                    <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider text-right">Valeur B</th>
                    <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider text-right">Taux (%)</th>
...
<div id="addCnamModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl">
        <h3 class="text-lg font-bold text-[#1e293b] mb-4">Ajouter un code CNAM</h3>
        <form method="POST" action="{{ route('center.cnam.store') }}" class="space-y-3">
            @csrf
            <div><label class="text-[10px] font-bold text-[#64748b] uppercase">Code CNAM</label>
            ...
            <div><label class="text-[10px] font-bold text-[#64748b] uppercase">Nom de l'examen</label>
            ...
            <div><label class="text-[10px] font-bold text-[#64748b] uppercase">Valeur B (TND)</label>
            ...
            <div><label class="text-[10px] font-bold text-[#64748b] uppercase">Taux CNAM (%)</label>
            ...
            <div><label class="text-[10px] font-bold text-[#64748b] uppercase">Description</label>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="bg-[#7C3AED] hover:bg-[#6D28D9] text-white font-bold px-4 py-2.5 rounded-xl text-sm transition">Ajouter</button>
                <button type="button" onclick="document.getElementById('addCnamModal').classList.add('hidden')" class="text-[#64748b] font-medium px-4 py-2.5 text-sm">Annuler</button>
            </div>
        </form>
    </div>
</div>
