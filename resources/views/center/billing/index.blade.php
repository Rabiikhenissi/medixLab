...
@section('title', 'Facturation - Medix eSanté')

@section('content')
<div class="space-y-6 select-none">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-[#1e293b]">Facturation</h1>
            <p class="text-sm text-[#64748b] mt-2">Gérez les factures et les paiements.</p>
        </div>
        ...
        <table class="w-full text-left border-collapse min-w-[800px]">
            <thead>
                <tr class="bg-[#F8FAFC]/80 border-b border-[#e2e8f0]/80">
                    <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">N° Facture</th>
                    <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Patient</th>
                    <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider text-right">Montant</th>
                    <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider text-right">Part CNAM</th>
                    <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider text-right">Part Patient</th>
                ...
                <tr class="hover:bg-[#F8FAFC]/70 transition">
                    ...
                    <td class="p-4 text-center">
                        @if($invoice->status === 'paid')
                            <span class="px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700">Payée</span>
                        @elseif($invoice->status === 'partially_paid')
                            <span class="px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700">Partielle</span>
                        @elseif($invoice->status === 'cancelled')
                            <span class="px-2.5 py-1 rounded-full text-[10px] font-bold bg-red-100 text-red-700">Annulée</span>
                        @else
                            <span class="px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-100 text-slate-700">En attente</span>
                        @endif
                    </td>
                    <td class="p-4 text-[#64748b]">{{ $invoice->created_at->format('d/m/Y') }}</td>
                    <td class="p-4 text-right">
                        <a href="{{ route('center.billing.show', $invoice->id) }}" class="text-[#7C3AED] hover:text-[#5B21B6] font-bold text-xs">Voir</a>
                    </td>
                </tr>
                @empty
                <tr><td colspan="8" class="p-8 text-center text-[#94a3b8]">Aucune facture.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
