...
@section('title', 'Gestion du Matériel - Medix eSanté')
...
@section('content')
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
                <thead>
                    <tr class="bg-[#F8FAFC]/80 border-b border-[#e2e8f0]/80">
                        ...
                        <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Numéro de Série</th>
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    @forelse($equipment as $item)
                        @php
                            ...
                            $statusMap = [
                                'pending' => ['label' => 'Signalé/En panne', 'color' => 'bg-red-50 text-red-600 border border-red-200'],
                                'in_progress' => ['label' => 'Sous maintenance', 'color' => 'bg-amber-50 text-amber-600 border border-amber-200'],
                                'completed' => ['label' => 'Terminé/Réparé', 'color' => 'bg-emerald-50 text-emerald-600 border border-emerald-200'],
                                'cancelled' => ['label' => 'Annulé', 'color' => 'bg-gray-50 text-gray-600 border border-gray-200'],
                            ];
                        ...
                        <tr class="hover:bg-[#F8FAFC]/50 transition">
                            ...
                            <td class="p-4 text-right">
                                <div class="inline-flex gap-2">
                                    <button onclick="openMaintenanceModal({{ $item->id }}, '{{ addslashes($item->name) }}')" class="px-3 py-1.5 bg-[#7C3AED]/10 hover:bg-[#7C3AED]/20 text-[#7C3AED] font-bold text-xs rounded-lg transition" title="Maintenance">
                                        Maintenance
                                    </button>
                                    <button onclick="openEditEquipmentModal({{ $item->id }}, '{{ addslashes($item->name) }}', '{{ addslashes($item->type) }}', '{{ $item->serial_number }}', '{{ $item->status }}')" class="p-1.5 border border-[#e2e8f0] text-gray-400 hover:text-[#7C3AED] hover:border-[#7C3AED]/30 rounded-lg transition" title="Modifier">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="p-8 text-center text-gray-400">Aucun matériel enregistré.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
                <thead>
                    <tr class="bg-[#F8FAFC]/80 border-b border-[#e2e8f0]/80">
                        ...
                        <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Date Fin</th>
                        <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Matériel</th>
                        <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Statut</th>
                        <th class="p-4 text-[10px] font-bold text-[#64748b] uppercase tracking-wider">Description</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 text-sm">
                    @forelse($maintenances as $log)
                        ...
                        @endphp
                        <tr class="hover:bg-[#F8FAFC]/50 transition">
                            <td class="p-4 text-xs font-medium text-[#475569]">{{ optional($log->start_date)->format('d/m/Y') ?? '-' }}</td>
                            <td class="p-4 text-xs font-medium text-[#475569]">{{ optional($log->end_date)->format('d/m/Y') ?? '-' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="p-8 text-center text-gray-400">Aucun historique de maintenance.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="equipment-modal" class="fixed inset-0 z-50 items-center justify-center bg-black/40 backdrop-blur-xs hidden">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl relative mx-4">
            ...
            <h3 class="text-sm font-bold text-[#1e293b] uppercase tracking-wider mb-4 pb-2 border-b border-gray-100">Ajouter du Matériel</h3>
            <form action="{{ route('center.equipment.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label for="name" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Nom du matériel</label>
                    <input type="text" name="name" id="name" required placeholder="Ex: ECG Machine" class="w-full mt-1 px-3 py-2 border border-[#e2e8f0] rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/10 focus:border-[#7C3AED] bg-white text-[#1e293b]">
                </div>
                <div>
                    <label for="type" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Type / Catégorie</label>
                    <input type="text" name="type" id="type" required placeholder="Ex: Diagnostic, Laboratoire" class="w-full mt-1 px-3 py-2 border border-[#e2e8f0] rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/10 focus:border-[#7C3AED] bg-white text-[#1e293b]">
                </div>
                <div>
                    <label for="serial_number" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Numéro de Série</label>
                    <input type="text" name="serial_number" id="serial_number" required placeholder="Ex: SN-2023-001" class="w-full mt-1 px-3 py-2 border border-[#e2e8f0] rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/10 focus:border-[#7C3AED] bg-white text-[#1e293b]">
                </div>
                <div class="flex justify-end space-x-3 pt-2">
                    <button type="button" onclick="closeEquipmentModal()" class="px-4 py-2 border border-gray-200 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-50 uppercase tracking-wider">Annuler</button>
    ...
    <div id="edit-equipment-modal" class="fixed inset-0 z-50 items-center justify-center bg-black/40 backdrop-blur-xs hidden">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl relative mx-4">
            ...
            <h3 class="text-sm font-bold text-[#1e293b] uppercase tracking-wider mb-4 pb-2 border-b border-gray-100">Modifier le Matériel</h3>
            <form id="edit-equipment-form" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label for="edit_name" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Nom du matériel</label>
                ...
                <div>
                    <label for="edit_type" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Type / Catégorie</label>
                ...
                <div>
                    <label for="edit_serial_number" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Numéro de Série</label>
                ...
                <div>
                    <label for="edit_status" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Statut</label>
                    <select name="status" id="edit_status" required class="w-full mt-1 px-3 py-2 border border-[#e2e8f0] rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/10 focus:border-[#7C3AED] bg-white text-[#1e293b]">
                        <option value="pending">Signalé / En panne</option>
                        <option value="in_progress">Sous maintenance</option>
                        <option value="completed">Terminé / Réparé</option>
                        <option value="cancelled">Annulé</option>
                    </select>
                </div>
                <div class="flex justify-end space-x-3 pt-2">
                    <button type="button" onclick="closeEditEquipmentModal()" class="px-4 py-2 border border-gray-200 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-50 uppercase tracking-wider">Annuler</button>
    ...
    <div id="maintenance-modal" class="fixed inset-0 z-50 items-center justify-center bg-black/40 backdrop-blur-xs hidden">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl relative mx-4">
            ...
            <h3 class="text-sm font-bold text-[#1e293b] uppercase tracking-wider mb-4 pb-2 border-b border-gray-100">Enregistrer une Maintenance</h3>
            <form id="maintenance-form" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="text-xs font-bold text-[#475569] uppercase tracking-wider">Matériel</label>
                    <div id="maintenance-equipment-name" class="mt-1 text-sm font-bold text-[#7C3AED]"></div>
                </div>
                <div>
                    <label for="maintenance_status" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Statut</label>
                ...
                <div>
                    <label for="maintenance_start" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Date Début</label>
                ...
                <div>
                    <label for="maintenance_end" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Date Fin (optionnel)</label>
                ...
                <div>
                    <label for="maintenance_description" class="text-xs font-bold text-[#475569] uppercase tracking-wider">Description / Observations</label>
                ...
                <div class="flex justify-end space-x-3 pt-2">
                    <button type="button" onclick="closeMaintenanceModal()" class="px-4 py-2 border border-gray-200 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-50 uppercase tracking-wider">Annuler</button>
                    <x-button type="submit" color="purple" :fullWidth="false" class="!py-2 !px-5 !text-xs font-bold uppercase tracking-wider">Enregistrer</x-button>
                </div>
            </form>
        </div>
...
<script>
    ...
    // Maintenance from equipment list button
    function openMaintenanceModal(id, name) {
        const modal = document.getElementById('maintenance-modal');
        const form = document.getElementById('maintenance-form');
        document.getElementById('maintenance-equipment-name').textContent = name;
