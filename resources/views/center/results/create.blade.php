...
@section('title', 'Saisir les résultats - Medix eSanté')
...
<div class="max-w-4xl mx-auto py-8 px-4 select-none">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-[#64748b] mt-1.5 font-medium">
                Saisissez les valeurs des paramètres pour l'examen de <strong class="text-[#7C3AED]">{{ $item->examRequest->patient->user->first_name }} {{ $item->examRequest->patient->user->last_name }}</strong>.
            </p>
        </div>
    ...
    {{-- Main Form Card --}}
    <form method="POST" action="{{ route('center.results.store', $item) }}" class="space-y-6">
        @csrf
        ...
        <div class="bg-gradient-to-br from-[#7C3AED]/5 to-[#7C3AED]/10 border border-[#7C3AED]/15 rounded-2xl p-5 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="font-bold text-[#1e293b] text-base">{{ $item->exam->name }}</h3>
                    <p class="text-xs text-[#64748b] mt-0.5">Examen prescrit par le Dr. {{ $item->examRequest->doctor->user->first_name }} {{ $item->examRequest->doctor->user->last_name }}</p>
                </div>
            </div>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        @if($parameter->normal_range)
                            <span class="inline-flex items-center text-[10px] text-[#64748b] mt-1 bg-[#F8FAFC] border border-[#e2e8f0] px-2 py-0.5 rounded-md font-medium">
                                Référence: {{ $parameter->normal_range }}
                            </span>
                        @endif
                        @if($parameter->critical_low !== null || $parameter->critical_high !== null)
                            <span class="inline-flex items-center text-[10px] text-red-600 mt-1 bg-red-50 border border-red-200 px-2 py-0.5 rounded-md font-bold">
                                ⚠ Critique: {{ $parameter->critical_low ?? '-' }} - {{ $parameter->critical_high ?? '-' }}
                            </span>
                        @endif
                    ...
                    <div class="flex items-center gap-3">
                        <div class="relative w-full md:w-44">
                            <input
                                ...
                                data-critical-low="{{ $parameter->critical_low ?? '' }}"
                                data-critical-high="{{ $parameter->critical_high ?? '' }}"
                        ...
                        <div class="relative w-full md:w-36">
                            ...
                            >
                                ...
                                <option value="critical" {{ old("parameters.{$index}.status") === 'critical' ? 'selected' : '' }}>⚠ Critique</option>
                            </select>
                        </div>
        ...
        <div class="bg-white border border-[#e2e8f0] rounded-2xl p-6 shadow-xs">
            <div class="flex items-center justify-between mb-5">
                ...
                <span id="consumable-count-badge" class="hidden inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-black bg-[#7C3AED]/10 text-[#7C3AED] border border-[#7C3AED]/20">0 sélectionné(s)</span>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-2 pt-4 border-t border-[#e2e8f0]/60">
                <select id="consumable-select" class="flex-1 px-3 py-2.5 border border-[#e2e8f0] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/20 focus:border-[#7C3AED] text-sm bg-[#F8FAFC]/50 text-[#1e293b]">
                    <option value="">— Sélectionner un consommable à ajouter —</option>
                    @foreach($consumables as $consumable)
                        <option value="{{ $consumable->id }}" data-unit="{{ $consumable->unit }}" data-stock="{{ $consumable->quantity }}" data-name="{{ $consumable->name }}">
                            {{ $consumable->name }} — Stock disponible: {{ $consumable->quantity }} {{ $consumable->unit }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
            ...
            <div class="bg-white border border-[#e2e8f0] rounded-2xl p-5 shadow-xs">
                <h4 class="text-xs font-bold text-[#64748b] uppercase tracking-wider mb-4 flex items-center gap-2">
                    ...
                    Machines / Équipements utilisés
                </h4>

                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[260px] overflow-y-auto pr-1">
                    @if($equipment->isEmpty())
                        <p class="text-xs text-[#64748b] italic col-span-3">Aucune machine active dans ce laboratoire.</p>
                    @else
                        @foreach($equipment as $equip)
                            <label class="flex items-center gap-3 p-3 border border-[#e2e8f0] rounded-xl hover:border-[#7C3AED]/20 cursor-pointer transition select-none">
                                <input type="checkbox" name="equipment[]" value="{{ $equip->id }}" class="rounded text-[#7C3AED] focus:ring-[#7C3AED]/30"
                                    @if($item->exam->examEquipment->contains('equipment_id', $equip->id)) checked @endif
                                >
                                <div class="flex-1 min-w-0">
                                    <span class="text-sm font-bold text-[#1e293b] block truncate">
                                        {{ $equip->name }}
                                        @if($item->exam->examEquipment->contains('equipment_id', $equip->id))
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-extrabold uppercase bg-purple-50 text-[#7C3AED] border border-[#7C3AED]/20 ml-1.5">Suggéré</span>
                                        @endif
                                    </span>
                                    <span class="text-[10px] text-[#64748b] block mt-0.5">S/N: {{ $equip->serial_number ?? 'N/A' }} | Type: {{ $equip->type ?? 'N/A' }}</span>
                                </div>
                            </label>
                        @endforeach
                    @endif
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateCountBadge() {
        if (selected.length > 0) {
            countBadge.textContent = `${selected.length} sélectionné(s)`;
    ...
    function render() {
        ...
        if (selected.length === 0) {
            listContainer.innerHTML = `
                <div class="flex flex-col items-center justify-center py-8 text-[#94a3b8] border-2 border-dashed border-[#e2e8f0] rounded-2xl">
                    ...
                    <p class="text-sm font-semibold">Aucun consommable sélectionné</p>
                    <p class="text-xs mt-1 opacity-70">Utilisez le sélecteur ci-dessous pour en ajouter</p>
                </div>`;
            updateCountBadge();
            return;
        }
        ...
        selected.forEach((item, index) => {
            ...
            div.innerHTML = `
                ...
                <div class="flex items-center gap-3">
                    ...
                    <label class="text-[10px] font-bold text-[#94a3b8] uppercase tracking-wider hidden sm:block">Qté</label>
                    <div class="flex items-center border border-[#e2e8f0] rounded-xl overflow-hidden bg-white shadow-xs">
                        <button type="button"
                            class="qty-dec w-8 h-9 flex items-center justify-center text-[#64748b] hover:text-[#7C3AED] hover:bg-[#7C3AED]/5 transition font-bold text-base"
                            data-index="${index}">
                            −
                        </button>
                        <input type="number"
                            ...
                            data-index="${index}">
                        <button type="button"
                            class="qty-inc w-8 h-9 flex items-center justify-center text-[#64748b] hover:text-[#7C3AED] hover:bg-[#7C3AED]/5 transition font-bold text-base"
                            data-index="${index}">
                            +
                        </button>
                    </div>
        ...
        // Wire up stepper buttons and direct input
    ...
    btnAdd.addEventListener('click', function () {
        ...
        if (selected.some(item => item.id === val)) {
            // Flash the existing item instead of alerting
    ...
    // Load pre-suggested consumables
    ...
    document.querySelectorAll('.param-value-input').forEach(input => {
        input.addEventListener('input', function () {
            ...
            if (status) {
                ...
                this.classList.remove('border-red-400','border-green-400','border-amber-400','border-purple-500');
                if (badge) {
                    badge.classList.remove('hidden','bg-red-100','text-red-700','border-red-300','bg-amber-100','text-amber-700','border-amber-300','bg-green-100','text-green-700','border-green-300','bg-purple-100','text-purple-700','border-purple-300');
                }
                if (status === 'critical') {
                    this.classList.add('border-purple-500');
                    if (badge) { badge.textContent = '⚠ Critique'; badge.classList.add('bg-purple-100','text-purple-700','border','border-purple-300'); }
                } else if (status === 'high') {
