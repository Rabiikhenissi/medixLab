...
@section('title', __('center.results.create_title') . ' - Medix eSanté')
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-[#1e293b]">
                {{ __('center.results.create_title') }}
            </h1>
            <p class="text-sm text-[#64748b] mt-1.5 font-medium">
                {!! __('center.results.create_subtitle', ['patient' => '<strong class="text-[#7C3AED]">'.$item->examRequest->patient->user->first_name.' '.$item->examRequest->patient->user->last_name.'</strong>']) !!}
            </p>
        </div>
        ...
        <div class="bg-gradient-to-br from-[#7C3AED]/5 to-[#7C3AED]/10 border border-[#7C3AED]/15 rounded-2xl p-5 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-[#64748b] mt-0.5">{{ __('center.results.prescribed_by', ['doctor' => $item->examRequest->doctor->user->first_name.' '.$item->examRequest->doctor->user->last_name]) }}</p>
                </div>
            </div>
            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase bg-purple-50 text-[#7C3AED] border border-[#7C3AED]/20">
                {{ __('center.results.code', ['code' => $item->exam->code ?? 'N/A']) }}
            </span>
        </div>
                ...
                <div class="bg-white border border-[#e2e8f0] rounded-2xl p-5 shadow-xs hover:border-[#7C3AED]/30 transition flex flex-col md:flex-row md:items-center gap-4">
                    <div class="flex-1 min-w-0">
                        ...
                        @if($parameter->normal_range)
                            <span class="inline-flex items-center text-[10px] text-[#64748b] mt-1 bg-[#F8FAFC] border border-[#e2e8f0] px-2 py-0.5 rounded-md font-medium">
                                {{ __('center.results.reference', ['range' => $parameter->normal_range]) }}
                            </span>
                        @endif
                        @if($parameter->critical_low !== null || $parameter->critical_high !== null)
                            <span class="inline-flex items-center text-[10px] text-red-600 mt-1 bg-red-50 border border-red-200 px-2 py-0.5 rounded-md font-bold">
                                {{ __('center.results.critical', ['low' => $parameter->critical_low ?? '-', 'high' => $parameter->critical_high ?? '-']) }}
                            </span>
                        @endif
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="relative w-full md:w-36">
                            ...
                            >
                                <option value="normal" {{ old("parameters.{$index}.status") === 'normal' ? 'selected' : '' }}>{{ __('center.results.normal') }}</option>
                                <option value="high" {{ old("parameters.{$index}.status") === 'high' ? 'selected' : '' }}>{{ __('center.results.high') }}</option>
                                <option value="low" {{ old("parameters.{$index}.status") === 'low' ? 'selected' : '' }}>{{ __('center.results.low') }}</option>
                                <option value="critical" {{ old("parameters.{$index}.status") === 'critical' ? 'selected' : '' }}>{{ __('center.results.critical_badge') }}</option>
                            </select>
                        </div>
        ...
        input.addEventListener('input', function () {
            ...
            if (status) {
                ...
                if (status === 'critical') {
                    ...
                    if (badge) { badge.textContent = @json(__('center.results.critical_badge')); badge.classList.add('bg-purple-100','text-purple-700','border','border-purple-300'); }
                } else if (status === 'high') {
                    this.classList.add('border-red-400');
                    if (badge) { badge.textContent = @json(__('center.results.high_badge')); badge.classList.add('bg-red-100','text-red-700','border','border-red-300'); }
                } else if (status === 'low') {
                    this.classList.add('border-amber-400');
                    if (badge) { badge.textContent = @json(__('center.results.low_badge')); badge.classList.add('bg-amber-100','text-amber-700','border','border-amber-300'); }
                } else {
                    this.classList.add('border-green-400');
                    if (badge) { badge.textContent = @json(__('center.results.normal_badge')); badge.classList.add('bg-green-100','text-green-700','border','border-green-300'); }
                }
            }
        });
