<x-layouts.doctor>
<x-slot:title>{{ __('doctor.medical_records.title') }} — {{ $patientUser->first_name }} {{ $patientUser->last_name }}</x-slot:title>
...
<div class="w-full max-w-[1100px] mx-auto py-8 px-4">
    ...
    <a href="{{ route('doctor.my-patients') }}"
        ...
        {{ __('doctor.medical_records.back_to_patients') }}
    </a>
        ...
        <div class="flex flex-col sm:flex-row items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-xs text-[#64748b]">
                    <span>{{ __('doctor.code_label') }} <strong>{{ $patient->patient_code }}</strong></span>
                    @if($patient->gender)<span>· {{ __('auth.gender_sex') }} : <strong>{{ $patient->gender === 'M' ? __('auth.gender_m') : __('auth.gender_f') }}</strong></span>@endif
                    @if($patient->date_of_birth)<span>· {{ __('doctor.born_label') }} : <strong>{{ $patient->date_of_birth->format('d/m/Y') }} ({{ $patient->date_of_birth->age }} {{ __('doctor.years_old') }})</strong></span>@endif
                    @if($patient->blood_group)<span>· {{ __('doctor.blood_label') }} : <strong>{{ $patient->blood_group }}</strong></span>@endif
                </div>
            </div>
    ...
    <div class="bg-white border border-[#e2e8f0] rounded-2xl p-6 md:p-8 shadow-sm">
        <h2 class="text-sm font-bold text-[#1e293b] mb-5">{{ __('doctor.medical_records.exam_history') }}</h2>

        @forelse($examRequests as $er)
            <div class="mb-4 last:mb-0 p-4 bg-[#f8fafc] rounded-xl border border-[#e2e8f0]">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                    <div class="flex items-center gap-2">
                        <span class="text-xs font-bold text-[#64748b]">{{ __('doctor.medical_records.request_no', ['id' => $er->id]) }}</span>
                        ...
                        @if($er->approved_by_doctor)
                            <span class="px-2 py-0.5 rounded-full text-[9px] font-bold bg-blue-50 text-blue-700 border border-blue-200">{{ __('doctor.validated') }}</span>
                        @endif
                    </div>
                ...
                @if($er->laboratory)
                <p class="text-[10px] text-[#94a3b8] mb-3">{{ __('doctor.lab_label') }} : <strong class="text-[#64748b]">{{ $er->laboratory->name }}</strong></p>
                @endif
                    ...
                    @foreach($er->items as $item)
                        <div class="pl-3 border-l-2 {{ $item->resultLabo ? 'border-emerald-400' : 'border-[#e2e8f0]' }}">
                            <div class="flex items-center justify-between">
                                ...
                                @if($item->resultLabo)
                                    <span class="px-2 py-0.5 rounded text-[9px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">{{ __('doctor.results_available') }}</span>
                                @else
                                    <span class="px-2 py-0.5 rounded text-[9px] font-bold bg-slate-50 text-slate-400 border border-slate-200">{{ __('doctor.status_pending') }}</span>
                                @endif
                            </div>
                            @if($item->resultLabo && $item->resultLabo->details->count() > 0)
                                ...
                                @if($item->resultLabo->interpretation)
                                    <p class="mt-2 text-[10px] text-[#64748b] italic">{{ __('doctor.interpretation_lab_label') }} {{ $item->resultLabo->interpretation }}</p>
                                @endif
                            @endif
                        </div>
                    @endforeach
                ...
                @if($er->doctor_interpretation)
                    <div class="mt-3 p-3 bg-blue-50/50 border border-blue-100 rounded-xl">
                        <p class="text-[9px] font-bold text-blue-600 uppercase tracking-wider mb-1">{{ __('doctor.my_interpretation') }}</p>
        ...
        @empty
            <div class="text-center py-10">
                ...
                <p class="text-sm font-semibold text-[#94a3b8]">{{ __('doctor.no_exam') }}</p>
                <p class="text-xs text-[#94a3b8] mt-1">{{ __('doctor.medical_records.no_exams') }}</p>
            </div>
        @endforelse
    </div>
