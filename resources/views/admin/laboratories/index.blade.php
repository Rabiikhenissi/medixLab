...
@section('title', __('admin.laboratories.index_title'))

@section('page-title', __('admin.laboratories.title_plural'))
@section('page-subtitle', __('admin.laboratories.page_subtitle'))
...
@section('header-actions')
    <a href="{{ route('admin.laboratories.create') }}" class="btn-add-exam">
        ...
        {{ __('admin.laboratories.add_title') }}
    </a>
@endsection
...
@section('content')
    <div class="data-section anim anim-1">
        ...
        <div class="data-header">
            <div class="data-title">{{ __('admin.laboratories.list_title') }}</div>
        </div>
        ...
        <form method="GET" action="{{ route('admin.laboratories.index') }}" id="filter-form">
            <div class="filters-bar">
                <!-- Search -->
                <div>
                    <span class="filter-label">{{ __('admin.laboratories.quick_search') }}</span>
                    <div class="filter-group" style="position:relative;display:inline-block;">
                        ...
                        <input type="text" name="search" value="{{ $search }}"
                            placeholder="{{ __('admin.laboratories.search_placeholder') }}" class="filter-input">
                    </div>
                </div>
                ...
                <div style="align-self:flex-end;">
                    <label class="filter-checkbox-wrap">
                        ...
                        {{ __('admin.laboratories.show_archived') }}
                    </label>
                </div>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    <th>{{ __('admin.laboratories.name_label') }}</th>
                    <th>{{ __('admin.laboratories.city') }}</th>
                    <th>{{ __('admin.laboratories.country') }}</th>
                    <th>{{ __('admin.laboratories.email_phone') }}</th>
                    <th>{{ __('admin.laboratories.physical_address') }}</th>
            ...
            <tbody>
                @forelse($laboratories as $labo)
                    <tr class="{{ $labo->is_archive ? 'archived' : '' }}">
                        ...
                        <td>
                            <span class="category-badge cat-biochemistry" style="font-weight: 700;">{{ $labo->city ?? __('admin.laboratories.not_provided') }}</span>
                        </td>
                        <td>
                        ...
                        <td>
                            @if ($labo->is_archive)
                                <span class="status-badge status-archived"><span class="dot"></span>{{ __('admin.laboratories.archived') }}</span>
                            @else
                                <span class="status-badge status-active"><span class="dot"></span>{{ __('admin.laboratories.active') }}</span>
                            @endif
                        </td>
                        <td style="text-align:right;">
                            <div style="display:flex;align-items:center;justify-content:flex-end;gap:6px;">
                                ...
                                <form action="{{ route('admin.laboratories.destroy', $labo) }}" method="POST"
                                    ...
                                    onsubmit="return swalConfirmSubmit(this, '{{ $labo->is_archive ? __('admin.laboratories.restore_confirm') : __('admin.laboratories.archive_confirm') }}')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="table-action-btn {{ $labo->is_archive ? 'restore-btn' : 'archive-btn' }}"
                                        title="{{ $labo->is_archive ? __('admin.laboratories.restore') : __('admin.laboratories.archive') }}">
                                        @if ($labo->is_archive)
                                ...
                                @if($labo->is_archive)
                                    <form action="{{ route('admin.laboratories.force-delete', $labo) }}" method="POST" style="display:inline;margin:0;"
                                          onsubmit="return swalConfirmSubmit(this, '{{ __('admin.laboratories.force_delete_confirm') }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="table-action-btn delete-btn" title="{{ __('admin.laboratories.force_delete') }}">
                                            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="empty-state">
                                ...
                                <h3>{{ __('admin.laboratories.empty_title') }}</h3>
                                <p>{{ __('admin.laboratories.empty_hint') }}</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
