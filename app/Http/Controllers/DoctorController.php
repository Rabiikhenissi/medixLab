...
class DoctorController extends Controller
{
...

    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Groupe d\'examens créé avec succès.',
            'exam_group_id' => $examGroup->id,
            'exam_group' => [
                'id' => $examGroup->id,
                'name' => $examGroup->name,
    ...
    public function applyExamGroup(Request $request)
    {
        ...
        // Verify the group belongs to the doctor
        if ($examGroup->doctor_id !== $doctor->id) {
            return response()->json([
                'success' => false,
                'message' => 'Ce groupe d\'examens ne vous appartient pas.',
        ...
        // Create exam request with group exams
        $examRequest = ExamRequest::create([
            'doctor_id' => $doctor->id,
            'patient_id' => $patient->id,
            'status' => 'pending',
        ...
        Notification::create([
            'user_id' => $patient->user_id,
            'title' => 'Nouvelle demande d\'analyses',
            'message' => 'Dr. ' . $doctor->user->first_name . ' ' . $doctor->user->last_name . ' vous a prescrit le groupe d\'examen "' . $examGroup->name . '" (' . $examGroup->items->count() . ' examen(s)).',
            'notification_type' => 'exam_request',
            'reference_id' => $examRequest->id,
        ]);
    ...
    /**
     * Update an existing exam group
     */
    public function updateExamGroup(Request $request, ExamGroup $examGroup)
    {
        $doctor = Auth::user()->doctor;
        ...
        return response()->json([
            'success' => true,
            'message' => 'Groupe d\'examens mis à jour avec succès.',
            'exam_group' => [
                'id' => $examGroup->id,
                'name' => $examGroup->name,
                'description' => $examGroup->description,
    ...
    /**
     * Delete an existing exam group
     */
    public function deleteExamGroup(ExamGroup $examGroup)
    {
        $doctor = Auth::user()->doctor;
        ...
        $examGroup->update(['is_archive' => true]);

        return response()->json([
            'success' => true,
            'message' => 'Groupe d\'examens supprimé avec succès.',
        ]);
    }
}
